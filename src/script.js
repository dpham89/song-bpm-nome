...
var adjustTempoBtn = document.getElementById('adjust-tempo');
...
document.querySelector('form').addEventListener('submit', function (formEvent) {
    ...
    queryInput.blur();
    adjustTempoBtn.innerText = '50%';
});


// Half/Restore tempo
adjustTempoBtn.addEventListener('click', () => {
    if (adjustTempoBtn.innerText === '50%') {
        tempo = tempo / 2;
        bpmInput.MaterialSlider.change(tempo);
    ...
    adjustTempoBtn.innerText = adjustTempoBtn.innerText === '50%' ? '100%' : '50%';
})

// Play found track by hitting space
window.addEventListener('keydown', function (e) {
    if (e.keyCode == 32 && e.target == document.body && e.target != queryInput) {
        e.preventDefault();
        playButton.click();
    }
});